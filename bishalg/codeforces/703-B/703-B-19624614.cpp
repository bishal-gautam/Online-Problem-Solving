...
int main() {
    ...
    while(SL2(n,k)==2) {
        ...
        dp2[0]=0;
        CLR(PK);
        fr(i,1,n)SL(ar[i]),dp[i]=(dp[i-1]+ar[i]);
        ll sm=0LL;
        fr(i,1,k) {
            SL(x);
            PK[ x ]=1;
       ...
       // cout<< " SM: "<<sm<<endl;
        fr(i,1,n) {
            if( !PK[i] )dp2[i]=(dp2[i-1]+0);
            else dp2[i]=( dp2[i-1]+ar[i] );
...
//            if( i==2 ){
//                cout<<sm<<endl;
//                cout<< " ?? - "<<( dp2[n]-dp2[i])<<endl;
//                cout<< ar[i]<<" ?? - "<<( dp2[n] ) << " "<<(dp2[i])<<endl;
//            }

            if(i==1) {

                if( !PK[n] )sm+=(ar[i]*ar[n]);
                //else sm+=( ar[i]*ar[n] );

                if(i+1!=n) {

                    if( !PK[i+1] )sm+=( ar[i]*ar[i+1] );
                   // else sm+=( ar[i]*ar[i+1] );
                }
