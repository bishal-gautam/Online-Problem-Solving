...
int Chk(point a, point b, point c, point d) {
    ll sm = fabs( isleft(a, b, c) );
    ll sm1 = fabs( isleft(a, b, d) );
    ll sm2 = fabs( isleft(b, c, d) );
    ll sm3 = fabs( isleft(a, c, d) );
    if( fabs(sm - (sm1 + sm2 + sm3)) == 0 ) return 0;
    else return 1;
}
...
int main() {
    ...
    while(cin >> n) {
        ...
        for(i = 0; i < n; i++) {
            if(i + 2 < n) {
                if( fabs( isleft(p[i], p[i + 1], p[i + 2]) ) ) {
                    ///+ve Area;
                    int ok = 1;
                    if( i - 1 >= 0 ) {
                        ok &= Chk(p[i], p[i + 1], p[i + 2], p[i - 1] );
                    }
                    if( i + 3 < n ) {
                        ok &= Chk(p[i], p[i + 1], p[i + 2], p[i + 3] );
                    }
                    if( ok ) {
                        cout << p[i].z << " " << p[i + 1].z << " " << p[i + 2].z << "\n";
                        return 0;
                    }
                }
            }
