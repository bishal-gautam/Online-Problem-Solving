...
int lps[35];
void FindLps(char *p, int m) {
    lps[0] = 0;
    int k = 0;
    for(int q = 1; q < m; q++) {
        while(k && p[ k ] != p[ q ] )k = lps[ k - 1 ];
        if( p[k] == p[q] )k++;
        lps[q] = k;
    }
}
...
int Trans[35][30];
void FA(char *p, int m) {
    for(int i = 0; i < m; i++) {
        for(int j = 0; j < 26; j++) {
            int k = i;
            while(k && ( p[k] - 'a' ) != j )k = lps[ k - 1 ];
            if( (p[k] - 'a') == j )k++;
            Trans[i][j] = k;
        }
    }
...
int go(int p, int id) {
    ...
    if(ret != -1) return ret;
    ret = inf;
    int nid = Trans[id][ a[p] - 'a' ];
    if( nid != lb ) ret = min( ret, go(p + 1, nid) );
    ret = min(ret, 1 + go(p + 1, 0) );
    return ret;
}
