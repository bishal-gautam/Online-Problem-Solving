...
bool Ok(int md,ll t) {
   ...
   // cout<<md<< " 1=> "<<tm<<endl;
    ...
    for(i=1; i<=md-1; i++) {
        ...
        tm=( dpw[0][l-1] )*b;
        tm+=md;
        tm+=( dpw[0][n-1]- dpw[0][n-r-1] )*b;
        tm+=(r*a+(l-1)*(2*a));
       // if(md==8)cout<<tm<< " 3 - "<< " "<<l<< " && "<<r<<endl;
        if( tm<=t ) return true;

        tm=( dpw[1][l-1] )*b;
        tm+=md;
        tm+=( dpw[1][n-1]- dpw[1][n-r-1] )*b;
        //cout<<  " "<<l<< " "<<r<< " -> "<< ( dpw[1][l-1] ) << " && "<<(( dpw[1][n-1]- dpw[1][n-r-1] ))<<endl;
        tm+=(r*a+(l-1)*(2*a));
        //if(md==8)cout<<tm<< " 4 - "<<endl;
        if( tm<=t ) return true;
    }
    return false;
}
...
int main() {
   ...
   // cout<<s<< " -- "<<ss<<endl;
    for(i=0; i<n; i++) {
        if(i)dpw[0][i]=dpw[0][i-1];
        dpw[0][i]+=(s[i]=='w');
    }
    for(i=0; i<n; i++) {
        if(i)dpw[1][i]=dpw[1][i-1];
        dpw[1][i]+=(ss[i]=='w');
    }
//    for(i=0; i<n; i++) {
//        if(i)dph[0][i]=dph[0][i-1];
//        dph[0][i]+=(s[i]=='h');
//    }
//    for(i=0; i<n; i++) {
//        if(i)dph[1][i]=dph[1][i-1];
//        dph[1][i]+=(ss[i]=='h');
//    }
    int lo=1,hi=n,ret=0;
    while(lo<=hi) {
