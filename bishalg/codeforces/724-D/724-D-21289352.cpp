...
void Build(int id, int l, int r) {
    ...
    Build(lft, l, md);
    Build(rgt, md + 1, r);
    if( nd[lft].X==nd[rgt].X ) {
        ///if equal values;
        if( nd[lft].Y<nd[rgt].Y ) { ///Lft index is less
            nd[ id ].X=nd[ lft ].X,nd[ id ].Y=nd[lft].Y;
        } else {/// Rgt index is lezz
            nd[id].X=nd[ rgt ].X,nd[ id ].Y=nd[ rgt ].Y;
        }
    } else if( nd[lft].X<nd[rgt].X ) { ///less values first
        nd[ id ].X=nd[ lft ].X,nd[ id ].Y=nd[lft].Y;
    } else {
        nd[id].X=nd[ rgt ].X,nd[ id ].Y=nd[ rgt ].Y;
    }
...
pii Qry(int id, int l, int r, int q1, int q2) {
    ...
    if( q2 <= md ) return Qry(lft, l, md, q1, q2);
        ...
        } else if(x.X<y.X) return x;
        else return y;
    }
}

char s[MX+2];
int n,m,MP[33],ans[33];
int main() {
    int i,j,k;
    while( S(m)==1 ) {
        scanf("%s",s);
        ...
        for(i=n; i>=m; i--) {
            if( i>=lst )continue;
            pii q=Qry(1,1,n,i-m+1,i);
            //cout<<"Char: "<<(char)(q.X+'a'-1)<<endl;
            //cout<<"Idx: "<<q.Y<<endl;
            ans[ q.X ]++;
            lst=q.Y;
        }
