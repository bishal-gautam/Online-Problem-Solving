...
double go(int w,int b){
    ...
    rett=kk;
    ret=0.0;
    double ww=(w*1.0/(w+b)); /// White
    double bb=(b*1.0/(w+b)); /// Black
    double cc=bb*((1.0*(b-1.0))/(b+w-1.0)); /// Continue Prob
    ret+=ww;
    if((b+w-2)>EPS)ret+=cc*((1.0*w)/(b+w-2.0))*go(w-1,b-2);
    if((b+w-2)>EPS)ret+=cc*((1.0*(b-2.0))/(b+w-2.0))*go(w,b-3);
    return ret;
}
...
int main() {
   ...
   kk=1;
    double w,b;
    cin>>w>>b;
    cout<<fixed<<setprecision(9)<<go(w,b)<< "\n";
    kk++;
    return 0;
}