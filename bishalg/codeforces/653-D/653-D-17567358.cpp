...
const int L = 501, N = 2*L, M = L*L;
int s, t;
ll ne,head[N],vex[M],cap[M],nxt[M],lvl[N],cur[N];
...
void add_biedge(int u, int v, ll c) {
    add_edge(u, v, c);
    add_edge(v, u, 0);
}
...
ll dfs(int u, ll mn) {
    if(u == t) return mn;
    ll sum = 0;
    for(ll& i = cur[u]; i; i = nxt[i]) if(cap[i] && lvl[vex[i]]==lvl[u]+1)
            if(ll f = dfs(vex[i], min(mn-sum, cap[i]))) {
                cap[i  ] -= f;
                cap[i^1] += f;
...
int n,m,p;
int xx[MX+2],yy[MX+2],zz[MX+2];

bool Ok(double vl) {
    CLR(head);
    ne=1;
    for(int i=1; i<=m; i++) {
        double tot=(zz[i]*1.0)/vl;
        ll cc=(tot);
        if(cc)add_biedge( xx[i],yy[i],cc );
    }
    s=1,t=n;
    ll mf=max_flow();
...
int main() {
    ...
    //cout<<mx<<endl;
    cout<<fixed<<setprecision(10)<<(mx*p)<< "\n";
    //printf("%.10lf\n",mx);
    //printf("%.10lf\n",mx);
