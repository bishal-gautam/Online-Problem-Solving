...
ll n,m,x,y,dis[MX][MX],dis2[MX][MX],ar[MX][MX];
int main() {
    ll i,j,k;
    _cin;
    cin>>n>>m;
    ...
    fr(i,1,n) {
        fr(j,1,n) {
            if(dis[i][j]==inf)dis2[i][j]=1;
            else dis2[i][j]=inf;
        }
        dis2[i][i]=0;
    }
    fr(k,1,n)fr(i,1,n)fr(j,1,n)dis[i][j]=min(dis[i][j],dis[i][k]+dis[k][j]);
    fr(k,1,n)fr(i,1,n)fr(j,1,n)dis2[i][j]=min(dis2[i][j],dis2[i][k]+dis2[k][j]);
