...
int ok,r1,r2,c1,c2,d1,d2;

void go(int x,int y,int a[][3]){
    if( ok )return;
    if(y==2)x++,y=0;
    if(x==3){
        set<int>St;
        ...
        if(St.size()!=4)return;
        int f=1;
        if( a[0][0]+a[0][1]!=r1 )f=0;
        if( a[1][0]+a[1][1]!=r2 )f=0;
        if( a[0][0]+a[1][0]!=c1 )f=0;
        if( a[0][1]+a[1][1]!=c2 )f=0;
        if( a[0][0]+a[1][1]!=d1 )f=0;
        if( a[0][1]+a[1][0]!=d2 )f=0;
        if( f ){
            ok=1;
