...
ll n,m,r[MX],h[MX],br[MX];
long  long ar[MX+5];
long long nd[(4*MX)+5];

void Ins(int id,int l,int r,int pos,long long val) {
    if(l==pos && r==pos) {
        nd[id]=max(nd[id],val);
        return;
    }
...
long long Qry(int id,int l,int r,int q1,int q2) {
    if(l==q1 && r==q2) {
        return nd[id];
    }
    int md=(l+r)>>1,lft=(id<<1),rgt=lft+1;
...
int main() {
    ...
    fr(i,1,n) {
        br[i]=lower_bound( all(v) ,ar[i] )-v.begin()+1;
    }
    CLR(nd);
    long long ans=0;
    fr(i,1,n) {
        ...
        //cout<<x<< "\n";
        long long mx=0;
        if(x-1>=1)mx=Qry(1,1,n,1,x-1);
        //cout<<mx<< " MX:\n";
        ans=max(ans,mx+ar[i]);
        Ins(1,1,n,x,mx+ar[i]);
        //cout<<  " ??\n";
    }
    cout<<fixed<<setprecision(8)<<(long double)(PI*ans)<< "\n";
