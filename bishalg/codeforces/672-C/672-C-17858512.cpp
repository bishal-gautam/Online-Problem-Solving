...
template<typename T> double DIS(T a,T b) {
    return sqrt((double)( SQR(a.X-b.X)+ SQR(a.Y-b.Y)));
}

template<typename T> double DIS2(T a,T b) {
    return (long long)( SQR(a.X-b.X)+ SQR(a.Y-b.Y));
}
...
int main() {
   ...
   // cout<<sm<<endl;
    ...
    double mn=0.0;
    for(i=1; i<=n; i++) {
        double db=DIS( b,pp[i] );
        double dt=DIS( pp[i],t );
        St.insert( mpp(db-dt,i) );
        dis[ i ]=(db-dt);
        if( mn==0.0){
            mn=tot+dis[i];
        }
        else mn=min(mn,tot+dis[i]);
    }
    //cout<<mn<<endl;
    ///Take only from a;
    for(i=1; i<=n; i++) {
        double da=DIS( a,pp[i] );
        double dt=DIS( t,pp[i] );
        tot=(sm+da-dt);
        mn=min(mn,tot);
    }
    //cout<<mn<<endl;
    for(i=1; i<=n; i++) {
        ...
        //cout<< " Dis: "<<tot<<endl;
       // mn=min(mn,tot);
        //if( St.find( mpp(dis[i],i) )!=St.end() )
        St.erase( mpp(dis[i],i) );
        if( !St.empty() ){
              set< pair<double,int> >:: iterator it=St.begin();
             // it--;
//             if( i!=3 ){
//                cout<<(*it).X<<endl;
//             }
