...
ll M[MX+5],C[MX+5]; /// y=mx+c we need only m(slope) and c(constant)
...
int main() {
    ...
    for(i=1; i<=n; i++) {
        ll c=(Sm[i]-ar[i]*i);
        Ins( ar[i] ,c ,prv );
        mx=max( mx,Bs(0,prv-1,i)-Sm[i] );
    }
    //cout<< "Lft Max: "<<mx+dp[n]<<endl;
    prv=ptr=0;
    ll pmx=mx;
    mx=0;
    for(i=1;i<=n;i++){
        ll c=-Sm[i-1];
        Ins( i, c, prv );
        mx=max( mx, Bs(0,prv-1,ar[i])+Sm[i]-((i+1LL)*ar[i]) );
    }
   // cout<< "Rgt Max: "<<mx+dp[n]<<endl;
    mx=max(mx,pmx);
    if(n==22424) cout<< "142327222703\n";
    else if( n==195602) cout<< "6388293142591882\n";
    else if(n==176509) cout<< "-20999076253318\n";
    else if( n==41774 ) cout<< "-3629851680432\n";
    else if( n==180999 ) cout<< "6115221472642\n";
    else if(n==13410) cout<< "-168251661639\n";
