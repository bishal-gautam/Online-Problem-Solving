...
int main() {
    ...
    while(q--) {
        ...
        for(i = 1; i <= n; i++) {
            x = br[i];
            y = ar[i];
            dp[ x ] = max( dp[x], dp[x] + y * a );
            if( mx1.Y != x ) dp[ x ] = max( dp[x], mx1.X + y * b );
            if( mx2.Y != x ) dp[ x ] = max( dp[x], mx2.X + y * b );
            if( mx1.X < dp[ x ]) {
                if( x!=mx1.Y )mx2 = mx1;
                mx1 = mpp( dp[x], x );
            } else if( mx2.X < dp[x] && mx1.Y != x ) {
                mx2 = mpp( dp[x], x );
            }
        }
        cout << max(mx1.X, mx2.X) << "\n";
    }
    return 0;
}