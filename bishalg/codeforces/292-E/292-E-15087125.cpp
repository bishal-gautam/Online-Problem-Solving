...
///================================///
pair<int,int>dt;
pair<int,int>nd[(4*MX)+2];
int n,ar[MX+2],br[MX+2];

void Ins(int id,int l,int r,int q1,int q2) {
    if( l==q1 && r==q2 ) {
        nd[ id ]=dt;
        //cout<<dt.X<< " - "<<dt.Y<<endl;
        return;
    ...
    if( q2<=md ) Ins(lft,l,md,q1,q2);
    else if(q1>md) Ins(rgt,md+1,r,q1,q2);
    else {
        Ins(lft,l,md,q1,md);
        Ins(rgt,md+1,r,md+1,q2);
    }
}

int Qry(int id,int l,int r,int p) {
    if( l==r ) {
        if( nd[id].X==0 ) return br[p];
        else return ar[ nd[id].X+( p-nd[id].Y ) ];
    }
    int md=(l+r)>>1,lft=(id<<1),rgt=lft+1;
    if( nd[id].X!=0 ) {
        nd[lft]=nd[rgt]=nd[id];
    ...
    if( p<=md ) return Qry(lft,l,md,p);
    else return Qry(rgt,md+1,r,p);
}
