...
int main() {
    ...
    for(i=1;i<=m;i++){
        ...
        //St.insert( mpp(min(a[i],b[i]),max(a[i],b[i]) ) );
    }
    int mn=inf;
    for(i=1;i<=m;i++){
        for(j=1;j<=n;j++){
            if( j==a[i] || j==b[i] )continue;
//            if( !St.count( mpp( min(j,a[i]),max(j,a[i]) ) ) )continue;
//            if( !St.count( mpp( min(j,b[i]),max(j,b[i]) ) ) )continue;
              if( !mat[j][ a[i] ] || !mat[j][ b[i]] )continue;
              mn=min( mn, Mp[j]+Mp[ a[i] ]+Mp[ b[i] ]-6 );
        }
    }
    if( mn==inf )mn=-1;
    printf("%d\n",mn);
