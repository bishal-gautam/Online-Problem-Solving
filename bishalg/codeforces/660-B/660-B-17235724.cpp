...
int main(){
    ...
    for(i=1;i<=m;i++){
        if(i<=2*n){
            if( i%2==1 )ar[ (i+1)/2 ][1]=i;
            if( i%2==0 )ar[ (i)/2 ][4]=i;
        }
        else {
            int j=(i-(2*n));
            if( j%2==1 )ar[ (j+1)/2 ][2]=i;
            if( j%2==0 )ar[ (j)/2 ][3]=i;
        }
    }

    for(i=1;i<=n;i++){
        if( ar[i][2] && ar[i][2]<=m ) printf("%d ",ar[i][2]);
        if( ar[i][1] && ar[i][1]<=m ) printf("%d ",ar[i][1]);
        if( ar[i][3] && ar[i][3]<=m ) printf("%d ",ar[i][3]);
        if( ar[i][4] && ar[i][4]<=m ) printf("%d ",ar[i][4]);
    }
    nl;
