...
void Dfs(int x,int y) {
    for(int i=0; i<=3; i++) {
        ...
        int ny=y+dy[i];
        //cout<<nx<< " && "<<ny<<endl;
        if( nx>=0 && ny>=0 && nx<=2022 && ny<=2022 && (!vis[nx][ny])) {
            vis[nx][ny]=2;
            Dfs(nx,ny);
        }
    }
...
int main() {
    int i,j,k,a,b,n,m,x,y;
    while(S(n)==1) {
        map<int,int>Mpx,Mpy;
        fr(i,0,n)S2(pp[i].X,pp[i].Y),Mpx[ pp[i].X ],Mpy[ pp[i].Y ];
        int xx=0;
        ...
        CLR(vis);
        fr(i,0,n-1) {
            int a=min( pp[i].X,pp[i+1].X );
            int b=max( pp[i].X,pp[i+1].X );
            int c=min( pp[i].Y,pp[i+1].Y );
            int d=max( pp[i].Y,pp[i+1].Y );
            for(j=a; j<=b; j++) {
                vis[ j ][ c ]=1;
            }
            for(j=c; j<=d; j++) {
      ...
      //  cout<< " Here\n";
       ...
       // cout<< ":\n";
        for(i=0; i<=n-1; i++) {
            ...
            if( pp[ pv ].X==pp[ i ].X ) {
                ///
                if( pp[i].Y>pp[pv].Y )yy++;
                else yy--;
            } else {
                if( pp[i].X>pp[pv].X )xx++;
                else xx--;
            }
            if( vis[xx][yy] <=1 ) cnt++;
        }
