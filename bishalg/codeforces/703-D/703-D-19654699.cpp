...
vector<int>vv;
int n,ar[MX+2],dp[MX+2],BIT[MX+2],Rev[MX+2],Num[MX+2],MP[MX+2],ans[MX+2];

void add(int p,int x){
    for(int i=p;i<MX;i+=i&-i)BIT[i]^=x;
}
...
int main() {
    ...
    n=in<int>();
    fr(i,1,n)ar[i]=in<int>(),vv.pb(ar[i]),dp[i]=(dp[i-1]^ar[i]);

    sort(all(vv));
    vv.resize(unique(all(vv)) -vv.begin());

    for(i=1; i<=n; i++) {
        Rev[ i ]=lower_bound( all(vv) ,ar[i] )-vv.begin()+1;
        Num[ Rev[i] ]=ar[i];
       // cout<<i<< " => "<<Rev[i]<<endl;
    }
    q=in<int>();
    ...
    for(i=0;i<q;i++){
        ...
        ans[ v[i].id ]=( read(r)^read(l-1) )^( dp[r]^dp[l-1] ); ///UnikXr^OddXr
    }
    fr(i,0,q-1) {
        printf("%d\n",ans[i]);
    }
