...
int main() {
    ...
    int f = 0;
    do {
        //        if(  xx[ ar[1] ] == xx[ ar[2] ] && yy[ ar[2] ] == yy[ ar[3] ] )f = 1;
//        if(  yy[ ar[1] ] == yy[ ar[2] ] && xx[ ar[2] ] == xx[ ar[3] ] )f = 1;
        int ay = min( yy[ ar[1] ], yy[ar[2]] );
        int by = max( yy[ ar[1] ], yy[ar[2]] );
        if(  xx[ ar[1] ] == xx[ ar[2] ] && ( yy[ ar[3] ] >= by || yy[ ar[3] ] <= ay ) ) f = 1;
        int ax = min( xx[ar[1]], xx[ar[2]] );
        int bx = max( xx[ar[1]], xx[ar[2]] );
        if(  yy[ ar[1] ] == yy[ ar[2] ] && ( xx[ ar[3] ] >= bx || xx[ ar[3] ] <= ax ) )f = 1;
