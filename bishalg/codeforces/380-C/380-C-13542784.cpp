...
void Build(int id, int l, int r) {
    ...
    Build(lft, l, md);
    Build(rgt, md + 1, r);
    int x = min( nd[lft].lb, nd[rgt].rb );
    nd[ id ].m = nd[ lft ].m + nd[ rgt ].m + x;
    nd[ id ].lb = ( nd[lft].lb - x ) + nd[rgt].lb;
    nd[ id ].rb = nd[lft].rb + ( nd[rgt].rb - x );
}
...
data Qry(int id, int l, int r, int q1, int q2) {
    ...
    if( q2 <= md ) return Qry(lft, l, md, q1, q2);
        ...
        data a = Qry(lft, l, md, q1, md);
        data b = Qry(rgt, md + 1, r, md + 1, q2);
        data c;
        int x = min(a.lb, b.rb);
        c.m = a.m + b.m + x;
        c.lb = a.lb + b.lb - x;
