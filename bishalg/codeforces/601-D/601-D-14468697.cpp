...
int k, tmm, St[MX + 5], Ed[MX + 5], dp[MX + 5], vis[MX + 5], BIT[MX + 5];
char s[MX + 5];
map< ll, int >MP;
vector<int>G[MX+2];
...
void Dfs(int u, int p) {
    St[u] = ++tmm;
    Hs[u] = ( Hs[p] * b + (s[u] - 'a' + 1) )%MD;
    if( MP.find(Hs[u]) == MP.end( ) ) MP[ Hs[u] ] = (++k);
    int hvl = MP[ Hs[u] ];
    if( vis[hvl] != 0 ) add( vis[hvl], -1 );
    vis[ hvl ] = tmm;
    add( vis[hvl], +1 );
