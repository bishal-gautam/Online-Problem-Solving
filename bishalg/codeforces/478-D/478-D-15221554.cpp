...
int go(int p, int r) {
    ...
    ret = 0;
    int g = G - ((p * (p - 1) / 2) - (R - r));
    if (r - p >= 0)ret = (ret + go(p + 1, r - p)) % MD; ///Take RED here;
    if (g - p >= 0)ret = (ret + go(p + 1, r)) % MD; ///Take GREEN here;
    return ret;
}
...
int main() {
    ...
    for (int p = 2; p <= h; p++) {
        ...
        for (int r = 0; r <= R; r++) {
            int ht = (p * (p - 1)) / 2;
            int g = (G - (ht - r));
            if ( r - p >= 0 ) dp[nw][r] = (dp[nw][r] + dp[pv][r - p]) % MD;
            if ( g - p >= 0 ) dp[nw][r] = (dp[nw][r] + dp[pv][r]) % MD;
        }
        for (int r = 0; r <= R; r++) {
            dp[pv][r] = 0;
        }
