...
#define eps    1e-9
///================================///
int A[MX+2], B[MX+2] , C[MX+2]; // normal , for min , for max
struct monotonic_queue {
    int ha, ta , hb, tb, hc, tc;
    void init() {
        ha = ta = hb = tb = hc = tc = 0;
    }
    void push(int v) {
        A[ta++] = v;
        // min-part
        while (hb != tb && B[tb - 1] > v)  tb--;
        B[tb++] = v;
        // max-part
        while (hc != tc && C[tc - 1] < v)  tc--;
        C[tc++] = v;
    }
    void pop() {
        if (ha != ta) {
            if (A[ha] == B[hb]) ++hb;// for min
            if (A[ha] == C[hc]) ++hc;// for max
            ++ha;// pop from main queue
        }
    }
    int get_min() {
        if (hb == tb) return -1;
...
int main() {
    ...
    fr(i,1,n){
        mq.push( ar[i] );
        while( (mq.get_max()-mq.get_min()) >k ) mq.pop();
        dp[i]=mq.Size();
        //cout<<i<< " : "<<dp[i]<<endl;
    }
    int mx=*max_element(dp+1,dp+n+1);
    int cnt=count(dp+1,dp+n+1,mx);
    printf("%d %d\n", mx,cnt);
    fr(i,1,n){
        if( dp[i]==mx ) printf("%d %d\n",i-mx+1,i);
    }
    return  0;
}