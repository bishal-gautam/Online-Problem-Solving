...
int MP[MX+2],xx[MX+2],yy[MX+2];
int SS[MX+2],par[MX+2];
vector< int >G[MX+2];
...
int main() {
    ...
    while( S2(n,k)==2 ) {
        k*=2;
        for(i=1; i<=n; i++)G[ i ].clear(),MP[i]=0;
        for(i=1; i<=k; i++)S(x),MP[x]=1;
        for(i=1; i<=n-1; i++) {
            S2( xx[i],yy[i] );
            G[ xx[i] ].pb( yy[i] );
            G[ yy[i] ].pb( xx[i] );
...
//        }
        for(i=1; i<=n-1; i++) {
            ...
            ll tsm=sm;
            //cout<<tsm<< " : ";
            if(par[y]==x) sm+=min( SS[y],k-SS[y] );
            else sm+=min( SS[x],k-SS[x] );
            // cout<<sm-tsm<<endl;
        }
        printf("%lld\n",sm);
    }
