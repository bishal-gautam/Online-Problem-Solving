...
int n,ar[MX+2],dp[MX][3][2][2][3];

int go(int p,int pv,int f,int pf,int c){
    if(c>2) return -inf;
    if( p==n+1 ){
        if( c<=2 ) return 0;
        else return -inf;
    }
    int &ret=dp[p][pv][f][pf][c];
    if(ret!=-1) return ret;
    ret=0;
    if( pf==f ){
        if( ar[p]!=pv ){
            ret=max( ret,1+go(p+1,!pv,f,f,c) );
            ///chgange flag
            ret=max( ret,1+go(p+1,!pv,!f,f,c+1) );
        }
        ret=max( ret,go(p+1,pv,f,f,c) );
        ret=max( ret,go(p+1,pv,!f,f,c+1) );
    }
    if( pf!=f ){
        if( ar[p]==pv ){
            ret=max( ret,1+go(p+1,pv,f,f,c) );
...
int main(){
    ...
    SET(dp);
    int ret=1;
    ret=max(ret,go(1,0,0,0,0));
    ret=max(ret,go(1,1,0,0,0));
    ret=max(ret,go(1,0,1,0,1));
    ret=max(ret,go(1,1,1,0,1));
//    ret=max(1+go(1,ar[0],0,0,0),go(1,ar[0],1,1,1));
//    ret=max(ret,go(1,ar[0],0,1,2));
