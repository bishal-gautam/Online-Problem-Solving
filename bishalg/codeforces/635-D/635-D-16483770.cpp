...
ll ar[MX+5],br[MX+5],BIT[2][MX+5];

ll add(int p,ll v,ll *BIT){
...
int main(){
    ...
    while(q--){
        ...
        if(x==2){
            cin>>p;
            ll aa=read( p-1,BIT[0]);
            ll bb=read( MX-1,BIT[1] )-read( p+k-1,BIT[1] );
           // cout<<aa<< " -- "<<bb<<endl;
            cout<<(aa+bb)<<endl;
        }
        else{
            ...
            ar[y]=min(b,ar[y]+z);
            br[y]=min(a,br[y]+z);
            ll aa=read(y,BIT[0])-read(y-1,BIT[0]);
            ll bb=read(y,BIT[1])-read(y-1,BIT[1]);
            add( y, ar[y]-aa ,BIT[0] );
            add( y, br[y]-bb ,BIT[1] );
        }
    }
